...
def monitor(cache_list, cache_dir_list, path):
    while(1):
        current_list = walk_subfile_list(path)
        current_dir_list = walk_subdir(path)
        #if len(cache_list) != len(current_list):
        find_difference(current_list, current_dir_list, cache_list, cache_dir_list, path)
...
if __name__ == '__main__':
    ...
    print(cache_list)
    monitor_process = Process(target=monitor, args=(cache_list, cache_dir_list, path,))
